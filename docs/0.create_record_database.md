...
df = pl.read_csv('CAMEO.country.txt', separator='\t')
...
df = (
    pl
    .scan_csv('GDELT.MASTERREDUCEDV2.TXT', separator='\t')
    .filter(
...
The `data/example.db` database is the final product, but for test purposes it's quite slow to work with. Particularly for plotting functions. This creates a second database with the sources reduced to just 50,000 records of New Zealand and 50,000 of Australia.

```bash
cp data/example.db data/example_mini.db
```
...
def count_records(conn):
    with Session(connection) as session:
        total = session.execute(select(GdeltRecord)).scalars().all()
    return len(total)

connection = DataInterface.open_connection('data/example_mini.db')

accepted_values = ['NZL', 'AUS']
group_size = 50_000

# Filter by GdeltRecord.source_id
with Session(connection) as session:
    stmt = delete(GdeltRecord).where(~GdeltRecord.source_id.in_(accepted_values))
    session.execute(stmt)
    session.commit()

assert count_records(connection) == 323_957 # Expected number of records with NZL or AUS as source

# Subsample
subq = (
    select(
        GdeltRecord.record_key,
        func.row_number()
        .over(
            partition_by=GdeltRecord.source_id,
...
stmt  = (
    delete(GdeltRecord)
    .where(
        GdeltRecord.record_key.in_(select(subq.c.record_key).where(subq.c.rn > group_size))
    )
)
...
assert count_records(connection) == 100_000
```
